...
from .prox import inplace_group_prox, inplace_prox
...
class LassoNetClassifier(BaseEstimator, ClassifierMixin):
    ...
    def load(self, state_snapshot):
        ...
        self.model.skip.weight.data = torch.from_numpy(state_snapshot["theta"]).to(
            self.device
        )
    ...
    def path(self, X, y, lambda_seq=None, validation_split=0.2, min_K=1):
        ...
        """
        self.fit(
            X, y, lambda_seq=lambda_seq, validation_split=validation_split, min_K=min_K
        )
    ...
    def fit(self, X, y, lambda_seq=None, validation_split=0.2, min_K=1):
        ...
        if self.lambda_start == "auto":
            ...
            self.lambda_start_ = (
                self.model.lambda_start(M=self.M) / optimizer.param_groups[0]["lr"] / 10
            )
        ...
        for lam, check_sparsity in path_iterator:
            ...
            if should_save:
                if self.verbose:
                    print(
                        f"Features active: {current_k} (lambda={lam:.5f}) Val Score: {val_score:.4f}"
                    )
                ...
                state_snapshot = {
                    ...
                    "lambda": lam,
                    "theta": copy.deepcopy(
                        self.model.skip.weight.detach().cpu().numpy()
                    ),
        ...
        if best_state is not None:
            if self.verbose:
                print(
                    f"Best model found with k={best_state['k']}, val_acc={best_state['val_acc']}"
                )
    ...
    def _train(
        ...
        def validation_obj(criterion):
            with torch.no_grad():
                ...
                return out_val, criterion(
                    out_val, y_val
                ).item() + lambda_val * self.model.l1_regularization_skip().item()
            ...
            def train_one_step(X_batch, y_batch):
                ...
                if lambda_val is not None:
                    with torch.no_grad():
                        ...
                        inplace_prox(
                            self.model.skip, self.model.layers[0], penalty, 0, self.M
                        )
